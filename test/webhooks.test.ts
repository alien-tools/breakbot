...
describe('BreakBot tests', () => {
  ...
  test('Opening a new PR creates and updates a new check run', async () => {
    ...
    await probot.receive({ id: 'synchronized', name: 'pull_request', payload: prOpenedPayload });
  });
  ...
  test('Re-running a check updates the check run', async () => {
    ...
    await probot.receive({ id: 'synchronized', name: 'check_run', payload: checkReRunPayload });
  });
